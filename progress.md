...
当前存在的问题是：我们是直接在index文件中fetch目录下的MD文件。这在SEO时会有风险，因此，我们不使用fetch进行预生成，而是提前制作出相应的HTML文件。

目前的想法是：使用pandoc的情况。构思好最终的相应命令，设置必要的参数。不少东西、不冗余。

而之后，在他帮我们生成好的一份HTML文件中，配置好我们相应的所需要的东西。例如，Markdown CSS主题，在里面重新配置好。（可以写2条，供我预览选择）


1.
仔细对比README文件，你会发现index文件中的body部分，有很多的排版上的不足，例如：带有列点的段落中，各个点并没有单独成行，而是整段撮合在一起。请进一步优化。

2.
在style部分中，进一步加入关于间距的设置，让间距显示的更加合理。让整一个文字排版更加清晰易读。增强呼吸感和可读性。

3.
进一步优化style中的其他设置，例如markdown CSS主题，请更换成另一个更加好看、美观的主流主题。例如Typora。换个CDN链接即可。